# -*- coding: utf-8 -*-
"""
Created on Sat May 11 19:15:03 2024
...
class DTFrame(ctk.CTkFrame):
    def __init__(self, master, write_func):
        ...
        self.rowconfigure(index=list(range(13)), weight=1, uniform="a")
        self.columnconfigure(index=(0, 1), weight=1, uniform="a")
#-------------------------------------------------------------------------------------------------
        self.label_column = ctk.CTkOptionMenu(self)
        self.label_column.grid(row=0, column=0, padx=5, pady=10, sticky="ew")
        self.label_column.set("Label")

        self.max_depth_ent = ctk.CTkEntry(self , placeholder_text = "Max Depth")
        self.max_depth_ent.grid(row=0, column=1, padx=5, pady=10, sticky="ew")
        
        self.tree_criterion = ctk.CTkOptionMenu(self , values = ["gini", "entropy", "log_loss"])
        self.tree_criterion.grid(row=1, column=0, padx=5, pady=10, sticky="ew")
        self.tree_criterion.set("Criterion")
        
        self.tree_splitter = ctk.CTkOptionMenu(self , values = ["best", "random"])
        self.tree_splitter.grid(row=1, column=1, padx=5, pady=10, sticky="ew")
        self.tree_splitter.set("Spliter")
        
        self.model_btn = ctk.CTkButton(self , text = "Make Model", command=self.Apply_model)
        self.model_btn.grid(row=2, column=0, padx=5, pady=10, sticky="ew" , columnspan =2)
        
        
        self.testsize_ent = ctk.CTkEntry(self , placeholder_text = "Test Size")
        self.testsize_ent.grid(row=3, column=0, padx=5, pady=10, sticky="ew")
        
        self.random_state_entry = ctk.CTkEntry(self , placeholder_text = "Random State")
        self.random_state_entry.grid(row=3, column=1, padx=5, pady=10, sticky="ew")
        ...
        self.rfe_model_btn.grid(row=6 , column=0, padx=5, pady=10, sticky="ew")
        
        self.rfe_model_entry = ctk.CTkEntry(self , placeholder_text = "RFE `N`" )
        self.rfe_model_entry.grid(row=6 , column=1, padx=5, pady=10, sticky="ew")
        

        
        self.smote_btn = ctk.CTkButton(self , text = "Apply SMOTE" , command = self.apply_smote)
        self.smote_btn.grid(row=7, column=0, padx=5, pady=10, sticky="ew", columnspan =2)        
        
        self.train_btn = ctk.CTkButton(self , text = "Train Model" , command = self.train_model)
        self.train_btn.grid(row=8, column=0, padx=5, pady=10, sticky="ew")

        self.test_btn = ctk.CTkButton(self , text = "Test Model"  , command = self.test_model)
        self.test_btn.grid(row=8, column= 1, padx=5, pady=10, sticky="ew")  
        ...
        self.plot_conf_btn.grid(row=12, column= 0, padx=5, pady=10, sticky="ew")
        
        self.plot_tree_btn = ctk.CTkButton(self , text = "Plot Tree" , command = self.plottree)
        self.plot_tree_btn.grid(row=12, column= 1, padx=5, pady=10, sticky="ew")
        
        self.tunning_btn = ctk.CTkButton(self , text = "Tunning" , command = self.tunning_algorithm)
        self.tunning_btn.grid(row=13, column= 0, padx=5, pady=10, sticky="ew" , columnspan=2)
...
#------------------------------------------------------------------------------------------------------
    ...
    def apply_rfe(self):
        ...
        self.model=self.dt
        n_feature =  int(self.rfe_model_entry.get())
    ...
    # Perform K-Fold cross-validation
        ...
        # Fit the DT model on the resampled training data
            self.dt.fit(self.x_train_res, self.y_train_res)
...
#------------------------------------------------------------------------------------------------------------------------------
    ...
    def plottree(self):
        # Create a new Tkinter window
        # Test plot separately
        fig, ax = plt.subplots(figsize=(10, 20))
        plot_tree(self.dt, feature_names=self.x.columns, class_names=self.y.unique().astype(str), filled=True, ax=ax)
        plt.show()

    def tunning_algorithm (self):
       self.params_dt = {'max_depth': np.arange(5, 30 , 5)}
       self.dt_gs = GridSearchCV(self.dt, self.params_dt, cv=5)
       self.dt_gs.fit(self.x_train, self.y_train)
       self.dt_best = self.dt_gs.best_estimator_
       self.show_detail_model(f'Best Paramter : {self.dt_gs.best_params_}')